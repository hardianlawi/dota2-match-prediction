...
@click.option(
    "--pub_matches_records", help="file containing the public matches records"
)
def main(api, less_than_match_id, pub_matches_records):
    ...
    if pub_matches_records is not None:
        assert (
            api == "matches_data"
        ), "Set API to matches data to query \
        pub matches records"
    ...
    if api == "pro_matches":
        ...
        if pub_matches_records is not None:
            ...
            filter_rule = {"match_id": {"$lt": less_than_match_id}}
            match_ids = [
                x["match_id"] for x in db.pro_matches.find(filter_rule, {"match_id": 1})
            ]
