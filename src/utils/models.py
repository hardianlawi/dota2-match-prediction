...
def define_model(
    ...
    if name == "nn_tanh":
        ...
        x = tf.keras.layers.Dense(
            1, activation="sigmoid", kernel_regularizer=tf.keras.regularizers.l2(alpha)
        )(x)
...
def train_and_pred(
    models, X_train, y_train, X_test, y_test, key, batch_size=128, epochs=60
):
    ...
    if isinstance(X_train, list) and isinstance(X_test, list):
        ...
        X_train = [
            radiant_scaler.fit_transform(X_train[0]),
            dire_scaler.fit_transform(X_train[1]),
        ]
