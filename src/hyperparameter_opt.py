...
unique_primary_attrs = ["agi", "int", "str"]
...
def load_data():
    ...
    assert np.all(
        radiants.columns == dires.columns
    ), "Radiants have different features than dires"
    assert (
        df.shape[0] == radiants.shape[0]
    ), "Number of matches in radiants are different to original matches data"
    assert (
        df.shape[0] == dires.shape[0]
    ), "Number of matches in dires are different to original matches data"
    ...
    radiants = radiants.rename(
        {col: "radiant_" + col for col in feature_names}, axis=1
    ).drop("match_id", axis=1)
    dires = dires.rename({col: "dire_" + col for col in feature_names}, axis=1).drop(
        "match_id", axis=1
    )
...
result = bayes_cv_tuner.fit(
    np.concatenate([radiants.values, dires.values], axis=1),
    df.match_id.values,
